...
impl FittedFeature for FittedLexicon {
    ...
    fn transform(&self, analysis: &Analysis<'_>, out: &mut VectorBuilder) {
        ...
        let lexical: Vec<(&str, Span)> = stream
            .lexical()
            .map(|(t, f)| (f, t.span))
            .collect();
...
impl FittedLexicon {
    fn structural_rules(&self, analysis: &Analysis<'_>, tokens: usize, out: &mut VectorBuilder) {
        ...
        for w in all.windows(6) {
            let pattern = [w[0].2, w[1].0 == ",", w[2].2, w[3].0 == ",", w[4].0 == "and", w[5].2];
...
type SeedTerm = (&'static str, &'static str, Severity, &'static [&'static str]);

const EXCESS_VOCAB: &[SeedTerm] = &[
    ("delve", "excess_vocab", Severity::High, &["dig into", "look at", "examine"]),
    ("delves", "excess_vocab", Severity::High, &["digs into", "examines"]),
    ("delving", "excess_vocab", Severity::High, &["digging into", "examining"]),
    ("intricate", "excess_vocab", Severity::Medium, &["complicated", "detailed"]),
    ("intricacies", "excess_vocab", Severity::Medium, &["details", "specifics"]),
    ("meticulous", "excess_vocab", Severity::Medium, &["careful", "thorough"]),
    ("meticulously", "excess_vocab", Severity::Medium, &["carefully"]),
    ("commendable", "excess_vocab", Severity::High, &["good", "worth doing"]),
    ("pivotal", "excess_vocab", Severity::Medium, &["key", "central"]),
    ("realm", "excess_vocab", Severity::Medium, &["field", "area"]),
    ("realms", "excess_vocab", Severity::Medium, &["fields", "areas"]),
    ("underscore", "excess_vocab", Severity::Medium, &["show", "stress"]),
    ("underscores", "excess_vocab", Severity::Medium, &["shows", "stresses"]),
    ("underscoring", "excess_vocab", Severity::Medium, &["showing"]),
    ("showcase", "excess_vocab", Severity::Medium, &["show", "demonstrate"]),
    ("showcases", "excess_vocab", Severity::Medium, &["shows"]),
    ("showcasing", "excess_vocab", Severity::Medium, &["showing"]),
    ("tapestry", "excess_vocab", Severity::High, &["mix", "range"]),
    ("nuanced", "excess_vocab", Severity::Medium, &["subtle", "careful"]),
    ("multifaceted", "excess_vocab", Severity::Medium, &["many-sided"]),
    ("holistic", "excess_vocab", Severity::Medium, &["overall", "whole"]),
    ("myriad", "excess_vocab", Severity::Medium, &["many", "countless"]),
    ("plethora", "excess_vocab", Severity::High, &["lots of", "many"]),
    ("unwavering", "excess_vocab", Severity::Medium, &["steady", "constant"]),
    ("profound", "excess_vocab", Severity::Medium, &["deep", "big"]),
    ("profoundly", "excess_vocab", Severity::Medium, &["deeply"]),
    ("foster", "excess_vocab", Severity::Medium, &["encourage", "build"]),
    ("fostering", "excess_vocab", Severity::Medium, &["encouraging"]),
    ("garner", "excess_vocab", Severity::Medium, &["get", "collect"]),
    ("garnered", "excess_vocab", Severity::Medium, &["got", "collected"]),
    ("harness", "excess_vocab", Severity::Medium, &["use"]),
    ("harnessing", "excess_vocab", Severity::Medium, &["using"]),
    ("elucidate", "excess_vocab", Severity::High, &["explain", "clarify"]),
    ("illuminate", "excess_vocab", Severity::Medium, &["explain", "show"]),
    ("embark", "excess_vocab", Severity::Medium, &["start", "begin"]),
    ("navigate", "excess_vocab", Severity::Medium, &["handle", "work through"]),
    ("navigating", "excess_vocab", Severity::Medium, &["handling"]),
    ("unveil", "excess_vocab", Severity::Medium, &["show", "reveal"]),
    ("unveiling", "excess_vocab", Severity::Medium, &["showing"]),
    ("spearhead", "excess_vocab", Severity::Medium, &["lead"]),
    ("streamline", "excess_vocab", Severity::Medium, &["simplify"]),
    ("streamlining", "excess_vocab", Severity::Medium, &["simplifying"]),
    ("bolster", "excess_vocab", Severity::Medium, &["strengthen", "back"]),
    ("augment", "excess_vocab", Severity::Medium, &["add to", "extend"]),
    ("amplify", "excess_vocab", Severity::Low, &["increase", "boost"]),
    ("cultivate", "excess_vocab", Severity::Medium, &["build", "grow"]),
    ("encompass", "excess_vocab", Severity::Medium, &["include", "cover"]),
    ("encompassing", "excess_vocab", Severity::Medium, &["covering"]),
    ("transformative", "excess_vocab", Severity::High, &["big", "far-reaching"]),
    ("groundbreaking", "excess_vocab", Severity::High, &["new", "novel"]),
    ("unparalleled", "excess_vocab", Severity::High, &["best", "unmatched"]),
    ("unprecedented", "excess_vocab", Severity::Medium, &["new", "first"]),
    ("invaluable", "excess_vocab", Severity::Medium, &["very useful"]),
    ("indispensable", "excess_vocab", Severity::Medium, &["essential"]),
    ("paramount", "excess_vocab", Severity::High, &["most important"]),
    ("quintessential", "excess_vocab", Severity::High, &["typical", "classic"]),
    ("resonate", "excess_vocab", Severity::Medium, &["connect", "land"]),
    ("resonates", "excess_vocab", Severity::Medium, &["connects"]),
    ("synergy", "excess_vocab", Severity::High, &["fit", "overlap"]),
    ("granular", "excess_vocab", Severity::Low, &["detailed"]),
    ("actionable", "excess_vocab", Severity::Low, &["usable", "concrete"]),
    ("scalable", "excess_vocab", Severity::Low, &["it scales"]),
    ("versatile", "excess_vocab", Severity::Low, &["flexible"]),
    ("revolutionize", "excess_vocab", Severity::High, &["change", "improve"]),
    ("facilitate", "excess_vocab", Severity::Medium, &["help", "enable"]),
    ("leverage", "excess_vocab", Severity::Medium, &["use"]),
    ("leveraging", "excess_vocab", Severity::Medium, &["using"]),
    ("robust", "excess_vocab", Severity::Low, &["solid", "reliable"]),
    ("seamless", "excess_vocab", Severity::Medium, &["smooth"]),
    ("seamlessly", "excess_vocab", Severity::Medium, &["smoothly"]),
    ("comprehensive", "excess_vocab", Severity::Low, &["complete", "full"]),
    ("adeptly", "excess_vocab", Severity::Medium, &["well", "skilfully"]),
    ...
    ("testament", "inflation", Severity::High, &["evidence", "sign"]),
    ("cornerstone", "inflation", Severity::Medium, &["basis", "foundation"]),
    ("backbone", "inflation", Severity::Medium, &["core"]),
    ("landscape", "inflation", Severity::Medium, &["field", "situation"]),
    ("ecosystem", "inflation", Severity::Low, &["set of tools", "community"]),
    ("paradigm", "inflation", Severity::Medium, &["model", "approach"]),
    ...
    ("undeniably", "hedge", Severity::Medium, &["clearly", "(cut it)"]),
...
type SeedPhrase = (&'static str, &'static str, &'static str, Severity, &'static str);

const SEED_PHRASES: &[SeedPhrase] = &[
    ("phrase.not_just_but", "not just * but *", "negative_parallelism", Severity::High,
     "negative parallelism: say the positive claim directly"),
    ("phrase.not_only_but_also", "not only * but also *", "negative_parallelism", Severity::High,
     "negative parallelism: say the positive claim directly"),
    ("phrase.its_not_x_its_y", "it's not * it's *", "negative_parallelism", Severity::High,
     "negative parallelism: say the positive claim directly"),
    ("phrase.worth_noting", "it is worth noting that", "filler", Severity::Medium,
     "delete the frame and state the fact"),
    ("phrase.worth_noting_contracted", "it's worth noting that", "filler", Severity::Medium,
     "delete the frame and state the fact"),
    ("phrase.important_to_note", "it is important to note", "filler", Severity::Medium,
     "delete the frame and state the fact"),
    ("phrase.plays_a_role", "plays a {crucial|vital|pivotal|key|significant|central} role",
     "inflation", Severity::High, "say what it actually does"),
    ("phrase.stands_as_testament", "stands as a testament", "inflation", Severity::High,
     "state the evidence instead"),
    ("phrase.ever_evolving", "ever evolving {landscape|world|field}", "inflation", Severity::High, ""),
    ("phrase.in_todays_world", "in today's {fast|digital|modern|connected} *", "inflation",
     Severity::Medium, ""),
    ("phrase.wide_range", "a wide {range|array|variety} of", "filler", Severity::Low, ""),
    ("phrase.delve_into", "delve into", "excess_vocab", Severity::High, "use \"look at\" or \"dig into\""),
    ("phrase.dive_into", "{let's|lets} {dive|delve} into", "excess_vocab", Severity::Medium, ""),
    ("phrase.shed_light", "shed light on", "filler", Severity::Medium, ""),
    ("phrase.pave_the_way", "pave the way", "inflation", Severity::Medium, ""),
    ("phrase.navigate_complexities", "navigate the {complexities|landscape|challenges}",
     "excess_vocab", Severity::High, ""),
    ("phrase.experts_argue", "experts {argue|say|believe|agree|note}", "vague_attribution",
     Severity::High, "name the expert or drop the claim"),
    ("phrase.studies_show", "studies {show|suggest|indicate|showed}", "vague_attribution",
     Severity::High, "cite the study or drop the claim"),
    ("phrase.some_argue", "some {argue|say|believe|claim}", "vague_attribution",
     Severity::Medium, "name who"),
    ("phrase.research_suggests", "research {suggests|shows|indicates}", "vague_attribution",
     Severity::High, "cite it or drop it"),
    ("phrase.one_of_the_most", "one of the most", "inflation", Severity::Low, ""),
    ("phrase.serves_as", "serves as {a|an|the}", "copula_avoidance", Severity::Medium, "use \"is\""),
    ("phrase.when_it_comes_to", "when it comes to", "filler", Severity::Medium, "cut the frame"),
    ("phrase.at_the_end_of_the_day", "at the end of the day", "filler", Severity::Medium, ""),
    ("phrase.in_conclusion", "in conclusion", "formulaic_close", Severity::High,
     "stop when you are done"),
    ("phrase.to_sum_up", "to sum up", "formulaic_close", Severity::Medium, ""),
    ("phrase.in_summary", "in summary", "formulaic_close", Severity::Medium, ""),
    ("phrase.that_being_said", "that being said", "filler", Severity::Low, ""),
    ("phrase.rest_assured", "rest assured", "filler", Severity::Medium, ""),
    ("phrase.look_no_further", "look no further", "filler", Severity::High, ""),
    ("phrase.unlock_the", "unlock the {power|potential|secrets|value} of", "inflation",
     Severity::High, ""),
    ("phrase.harness_the_power", "harness the power of", "inflation", Severity::High, ""),
    ("phrase.game_changer", "a game changer", "inflation", Severity::High, ""),
    ("phrase.no_denying", "there is no denying", "filler", Severity::Medium, ""),
    ("phrase.key_takeaway", "the key {takeaway|takeaways}", "formulaic_close", Severity::Low, ""),
    ("phrase.cutting_edge", "cutting edge", "inflation", Severity::Medium, ""),
    ("phrase.state_of_the_art", "state of the art", "inflation", Severity::Low, ""),
];
        ...
        LexiconPack {
            ...
            sources: vec![
                PackSource {
                    ...
                    note: "excess-vocabulary word list (Kobak et al., Science Advances 2025)".into(),
                },
                PackSource {
                    name: "Liang et al., ICML 2024".into(),
                    url: "https://arxiv.org/abs/2403.07183".into(),
                    note: "top adjectives and adverbs over-used in LLM-assisted peer reviews".into(),
                },
                PackSource {
...
mod tests {
    ...
    fn phrase_patterns_match_with_gaps_and_alternatives() {
        ...
        assert_eq!(v.get(i.get("lex:ai-slop:phrase.plays_a_role").unwrap()), 0.0);
    }
    ...
    fn alternatives_are_available_for_findings() {
        ...
        let sym = i.get("lex:ai-slop:word.delve").unwrap();
        assert!(fitted.alternatives_of(sym).contains(&"dig into".to_string()));
